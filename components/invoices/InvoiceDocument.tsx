...
import { decimalCompare } from "@/lib/invoices/decimal";
  ...
  return <article className={`invoice-a4 ${className || ""}`} dir="rtl" lang="ckb" data-paper="A4">
    ...
    <table className="invoice-items">
      ...
      <tbody>{data.items.map((item, index) => <tr key={`${item.sku || item.name}-${index}`}>
        ...
        <td className="invoice-quantity"><bdi>{formatQuantityWithUnit(item.quantity, config.showUnit ? item.unit : undefined)}</bdi></td>
    ...
    <section className="invoice-summary">
      <div className="invoice-totals">
        ...
        {config.showDiscount && decimalCompare(data.discount, 0) !== 0 ? <Field label={labels.discount} ltr>{formatReceiptMoney(data.discount, data.currency)}</Field> : null}
        {config.showTax && decimalCompare(data.tax, 0) !== 0 ? <Field label={labels.tax} ltr>{formatReceiptMoney(data.tax, data.currency)}</Field> : null}
...
function ThermalReceipt({ config, company, data, className }: Props) {
  ...
  return <article className={`invoice-thermal ${className || ""}`} dir="rtl" lang="ckb" data-paper="80mm">
    ...
    <div className="thermal-items">{data.items.map((item, index) => <div className="thermal-item" key={`${item.sku || item.name}-${index}`}><span>{index + 1}. {item.name}<small><bdi>{formatQuantityWithUnit(item.quantity, config.showUnit ? item.unit : undefined)}</bdi> × <bdi>{formatReceiptMoney(item.unitPrice, data.currency)}</bdi>{config.showDiscount && item.discount !== undefined ? ` · ${labels.discount}: ${formatReceiptMoney(item.discount, data.currency)}` : ""}</small></span><b dir="ltr">{formatReceiptMoney(item.total, data.currency)}</b></div>)}</div>
