...
function A4Receipt({ config, company, data, className }: Props) {
  ...
  return <article className={`invoice-a4 ${className || ""}`} dir="rtl" style={style} data-paper="A4">
    ...
    <section className="invoice-summary">
      <div className="invoice-balance"><b>{labels.remaining}:</b><strong className="invoice-ltr">{formatReceiptMoney(remaining, data.currency)}</strong>{config.showNotes && data.notes ? <p>{data.notes}</p> : null}</div>
      <div className="invoice-totals"><Field label={labels.subtotal} ltr>{formatReceiptMoney(data.subtotal, data.currency)}</Field><Field label={labels.discount} ltr>{formatReceiptMoney(data.discount, data.currency)}</Field>{config.showTax ? <Field label={labels.tax} ltr>{formatReceiptMoney(data.tax, data.currency)}</Field> : null}<Field label={labels.grandTotal} ltr>{formatReceiptMoney(data.total, data.currency)}</Field><Field label={labels.paid} ltr>{formatReceiptMoney(paid, data.currency)}</Field></div>
    </section>
...
function ThermalReceipt({ config, company, data, className }: Props) {
  ...
  return <article className={`invoice-thermal ${className || ""}`} dir="rtl" data-paper="80mm" style={{ fontFamily: config.fontFamily }}>
    ...
    <div className="thermal-totals"><div><span>{labels.grandTotal}</span><strong>{formatReceiptMoney(data.total, data.currency)}</strong></div><div><span>{labels.paid}</span><b>{formatReceiptMoney(paid, data.currency)}</b></div><div><span>{labels.remaining}</span><b>{formatReceiptMoney(Math.max(0, data.total - paid), data.currency)}</b></div></div>
